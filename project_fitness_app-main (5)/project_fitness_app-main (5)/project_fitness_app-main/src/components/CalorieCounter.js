...
function CalorieCounter() {
  ...
  const handleAddFood = () => {
    const selectedFood = foodData.find(food => food.name.toLowerCase() === foodName.toLowerCase());
  ...
  return (
    <div>
      <h2 style={{paddingLeft:'630px',paddingTop:'150px'}}>Calorie Counter</h2>
      <div style={{paddingLeft:'480px',paddingTop:'30px'}}>
        <label htmlFor="foodName" style={{paddingRight:'10px',fontWeight:'700',fontSize:'22px'}}>Food:</label>
        <TextField
          height="76px"
          sx={{ input: { fontWeight: '700', border: 'none', borderRadius: '4px' }, width: { lg: '400px', xs: '350px' }, backgroundColor: '#fff', borderRadius: '40px' }}
      ...
      <button onClick={handleAddFood} className='btn btn-bmi' style={{ textDecoration: 'none', width: '400px', textAlign: 'center', background: '#FF2625', fontSize: '22px', textTransform: 'none', color: 'white', borderRadius: '8px'  }}>Add Food</button>
      <div>
        <p style={{paddingLeft:'550px',paddingTop:'30px',fontSize:'22px',fontWeight:'700'}}>Total Calories: {calories}</p>
      </div>
    </div>
  );
}
